...
% APA Style (7th edition): apa7 document class + biblatex-apa (Biber)
% Output folder: APA7th_AEJMC (e.g., AEJMC / journal-style manuscript)
%
% Compile: pdflatex report && biber report && pdflatex report && pdflatex report
% Figures resolve via \graphicspath from parent project: ../results/figures/

% floatsintext: keep tables/figures in the body (default man mode uses endfloat and stacks them at the end)
...
% Figures live in the main project tree
...
\shorttitle{Nationalist Discourse on Bilibili}
...
\keywords{Chinese digital nationalism, Bilibili, natural language processing, RoBERTa, sentence embeddings, algorithmic amplification, computational social science}
...
This study contributes in four main respects. (1) An original corpus of 1.5 million Bilibili comments linked to 13,124 videos is assembled, covering a five-month controversy window (July--December 2025). (2) A multi-stage annotation and classification pipeline is specified and evaluated, combining GPT-4o-mini batch annotation, RoBERTa fine-tuning, and SBERT-based semantic clustering. (3) Systematic ablation studies are reported that compare three pretrained architectures, two input lengths, and binary versus multi-class formulations. (4) A \textit{null result reframing} is offered: contrary to an initial expectation that algorithms would preferentially amplify ideological extremity, the evidence indicates that platform engagement covaries with content density and analytical register, which sharpens the account of algorithmic incentive structures available to communication research.

\section{Literature Review}

\subsection{Platform Nationalism in China}
\textcite{schneider2018} documents how Chinese digital nationalism has evolved from spontaneous online protest toward organized, semi-professional content factories. \textcite{repnikova2018} analyze how platform architectures in China intertwine state interests with participatory user dynamics, creating a distinctive form of ``authoritarian participatory persuasion.'' \textcite{wu2022bilibili} specifically examines Bilibili as a site where youth nationalism intersects with gaming fandom culture, noting the platform's role in amplifying identity-laden commentary. Our work extends this tradition by applying large-scale computational methods to a single high-profile controversy.

\subsection{Hate Speech and Stance Detection}
\textcite{davidson2017hate} established foundational benchmarks for automated hate speech detection, highlighting the difficulty of distinguishing offensive language from targeted hate. \textcite{devlin2019bert} introduced BERT, which rapidly became the dominant architecture for Chinese NLP classification tasks; \textcite{cui2020macbert} further adapted it for Chinese via whole-word masking, and \textcite{liu2019roberta} proposed RoBERTa's robustly optimized pretraining strategy. We leverage the Chinese RoBERTa-wwm-ext variant for our classifier. For semantic embedding, we use Sentence-BERT \parencite{reimers2019sbert}, which constructs dense sentence representations via Siamese network fine-tuning, enabling cosine-similarity-based discourse analysis.

\subsection{Algorithmic Amplification}
\textcite{papacharissi2015} theorizes \textit{affective publics}---networked formations in which high-arousal emotional content circulates disproportionately. \textcite{bail2018} empirically demonstrate that exposure to opposing viewpoints on social media can paradoxically increase political polarization rather than moderate it. \textcite{gillespie2018} analyzes the structural role of content moderation in shaping platform discourse, arguing that algorithmic curation decisions are never ideologically neutral. Our RQ2 analysis extends these frameworks to the Chinese context, testing whether affective or ideological intensity predicts engagement in a censorship-mediated platform environment.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig1_hyperparam_heatmap.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig2_model_boxplot.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig4_maxlen_ablation.png}
...
\subsubsection{Cluster--Discourse-Type Alignment}
...
Nationalist discourse is not restricted to a single semantic zone. Cluster 6 functions as a concentration point, but the remaining nine clusters all carry nationalist content at rates between 1.3\% and 27.3\%, meaning the framing permeates the corpus broadly. This is consistent with the \textit{floating signifier} dynamic \textcite{hall1996} describes, where nationalist frameworks graft onto diverse topics rather than forming a fixed, recognizable territory.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{fig7_tsne.png}
...
\subsubsection{Intra-class Cosine Similarity}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig6_likes_boxplot.png}
